...
public class ReciprocalArraySumTest extends TestCase {
    ...
    private final static int REPEATS = 60;
    ...
    public void testParSimpleTwoMillion() {
        ...
        final double speedup = calculateAchievedSpeedUp(2_000_000, false, 2);
    ...
    public void testParSimpleTwoHundredMillion() {
        final double speedup = calculateAchievedSpeedUp(200_000_000, false, 2);
        final double minimalExpectedSpeedup = 1.5;
        final String errMsg = String.format("It was expected that the two-task parallel implementation would run at " +
                "least %fx faster, but it only achieved %fx speedup", minimalExpectedSpeedup, speedup);
        assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
    ...
    public void testParManyTaskTwoMillion() {
        ...
        final double speedup = calculateAchievedSpeedUp(2_000_000, true, nCores);
    ...
    public void testParManyTaskTwoHundredMillion() {
        final int nCores = ConcurrencyUtils.getThreadPoolSize();
        final double speedup = calculateAchievedSpeedUp(200_000_000, true, nCores);
     ...
     */
    private double calculateAchievedSpeedUp(final int N, final boolean useManyTaskVersion, final int nTasks) {
        ...
        if (useManyTaskVersion) {
            sum = ReciprocalArraySum.parallelManyTaskArraySum(input, nTasks);
        } else {
            assert nTasks == 2;
            sum = ReciprocalArraySum.parallelArraySum(input);
        }
        ...
        for (int r = 0; r < REPEATS; r++) {
            if (useManyTaskVersion) {
                ReciprocalArraySum.parallelManyTaskArraySum(input, nTasks);
            } else {
                assert nTasks == 2;
                ReciprocalArraySum.parallelArraySum(input);
            }
        }
